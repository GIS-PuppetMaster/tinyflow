...
class TrainExecutor(object):
    ...
    def run(self, feed_dict, Accuracy_node=None, convert_to_numpy_ret_vals=False):
        ...
        if self.isfirstrun == 0:
            ...
            # PCIE_bandwidth = nvmlDeviceGetPcieThroughput(handle, NVML_PCIE_UTIL_COUNT)  # KB/s
            #
            # PCIE_bandwidth *= 1024
            # pciin, pciout = PCIE_bandwidth, PCIE_bandwidth
            ...
            for idx in range(len(self.topo_order)):
                ...
                if node in self.Variable_node_list:
                    if index_to_gpu_map[idx] != None or schedule:
                        continue
                    else:
                        st = time.time()
                        # 没在GPU中,重新在GPU申请空间
                ...
                if node in self.mv:
                    if index_to_gpu_map[node.index] != None or schedule:
                        continue
                    else:
                        st = time.time()
                        # 没在GPU中,重新在GPU申请空间
    ...
    def tensor_accsess(self, access_node, idx):
        access_node.access_count += 1
        t = time.time()
        self.capu.add_tensor_access_info(access_node.index, access_node.access_count, t, idx)
